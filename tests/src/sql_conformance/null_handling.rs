...
fn test_null_is_null() {
    ...
    exec(&engine, "INSERT INTO t VALUES (3, 'world')");

    let n = count_rows(&engine, "SELECT id FROM t WHERE val IS NULL");
    assert_eq!(n, 1, "IS NULL should find exactly 1 row (id=2)");

    let result = engine.execute("SELECT id FROM t WHERE val IS NULL").unwrap();
...
fn test_null_is_not_null() {
    ...
    let n = count_rows(&engine, "SELECT id FROM t WHERE val IS NOT NULL");
...
fn test_null_equality_returns_null() {
    ...
    let engine = make_engine(dir.path());
    exec(&engine, "CREATE TABLE t (id INT, val TEXT)");
    exec(&engine, "INSERT INTO t VALUES (1, NULL)");
    exec(&engine, "INSERT INTO t VALUES (2, 'hello')");

    let n = count_rows(&engine, "SELECT id FROM t WHERE val = NULL");
    assert_eq!(n, 0, "NULL = NULL must be NULL/false, not TRUE — WHERE val = NULL matches no rows");

    let n2 = count_rows(&engine, "SELECT id FROM t WHERE val IS NULL");
...
fn test_null_not_equal_is_null() {
    ...
    let result = engine.execute("SELECT id FROM t WHERE val <> 'hello'").unwrap();
...
fn test_null_in_order_by() {
    ...
    exec(&engine, "INSERT INTO t VALUES (1, 10)");
    exec(&engine, "INSERT INTO t VALUES (2, NULL)");
    exec(&engine, "INSERT INTO t VALUES (3, 5)");

    let result = engine.execute("SELECT id FROM t ORDER BY val ASC").unwrap();
...
fn test_null_not_equal_to_zero() {
    ...
    let n = count_rows(&engine, "SELECT * FROM t WHERE val = 0");
...
fn test_null_insert_and_select() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    exec(&engine, "CREATE TABLE t (id INT, a TEXT, b INT)");
    exec(&engine, "INSERT INTO t VALUES (1, NULL, NULL)");
...
fn test_null_coalesce_returns_first_non_null() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT COALESCE(NULL, NULL, 42)").unwrap();
    match result.rows[0].get_by_idx(0) {
        Some(Value::Int4(42)) => {}
...
fn test_null_where_in_with_null_in_list() {
    ...
    let n = count_rows(&engine, "SELECT * FROM t WHERE val IN (1, NULL)");
...
fn test_null_arithmetic_add() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT 1 + NULL").unwrap();
...
fn test_null_arithmetic_multiply_zero() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT NULL * 0").unwrap();
...
fn test_null_arithmetic_subtract() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT NULL - NULL").unwrap();
...
fn test_null_coalesce_two_nulls_then_value() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT COALESCE(NULL, NULL, 'fallback')").unwrap();
    match result.rows[0].get_by_idx(0) {
...
fn test_null_coalesce_all_null() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT COALESCE(NULL, NULL)").unwrap();
...
fn test_null_nullif_equal() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT NULLIF(1, 1)").unwrap();
...
fn test_null_nullif_not_equal() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT NULLIF(1, 2)").unwrap();
    match result.rows[0].get_by_idx(0) {
        Some(Value::Int4(1)) => {}
...
fn test_null_in_case_when() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT CASE WHEN NULL THEN 'yes' ELSE 'no' END").unwrap();
    match result.rows[0].get_by_idx(0) {
...
fn test_null_string_concat() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT 'hello' || NULL").unwrap();
...
fn test_null_boolean_and_false() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT NULL AND FALSE").unwrap();
...
fn test_null_boolean_or_true() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT NULL OR TRUE").unwrap();
...
fn test_null_not_null() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = engine.execute("SELECT NOT NULL").unwrap();
...
fn test_null_order_by_nulls_last() {
    ...
    exec(&engine, "INSERT INTO t VALUES (1, 30)");
    exec(&engine, "INSERT INTO t VALUES (2, NULL)");
    exec(&engine, "INSERT INTO t VALUES (3, 10)");
    exec(&engine, "INSERT INTO t VALUES (4, 20)");

    let result = engine.execute("SELECT id FROM t ORDER BY score ASC").unwrap();
...
fn test_null_complex_where_is_null() {
    ...
    exec(&engine, "INSERT INTO t VALUES (1, 10, 'x')");
    exec(&engine, "INSERT INTO t VALUES (2, NULL, 'y')");
    exec(&engine, "INSERT INTO t VALUES (3, 30, NULL)");
    exec(&engine, "INSERT INTO t VALUES (4, NULL, NULL)");

    // Rows where a IS NULL: ids 2 and 4
    let n = count_rows(&engine, "SELECT id FROM t WHERE a IS NULL");
    assert_eq!(n, 2, "2 rows have a IS NULL");

    // Rows where a IS NULL AND b IS NOT NULL: only id=2
    let n2 = count_rows(&engine, "SELECT id FROM t WHERE a IS NULL AND b IS NOT NULL");
...
fn test_null_complex_where_is_not_null() {
    ...
    exec(&engine, "INSERT INTO t VALUES (2, NULL)");
    exec(&engine, "INSERT INTO t VALUES (3, 300)");

    let n = count_rows(&engine, "SELECT id FROM t WHERE val IS NOT NULL AND val > 50");
...
fn test_null_in_group_by() {
    ...
    exec(&engine, "INSERT INTO t VALUES ('a', 2)");
    exec(&engine, "INSERT INTO t VALUES (NULL, 3)");
    exec(&engine, "INSERT INTO t VALUES (NULL, 4)");
    exec(&engine, "INSERT INTO t VALUES ('b', 5)");
...
fn test_null_in_join_condition() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    exec(&engine, "CREATE TABLE left_t (id INT, key_col INT)");
    exec(&engine, "CREATE TABLE right_t (id INT, key_col INT)");
    exec(&engine, "INSERT INTO left_t VALUES (1, 10)");
    exec(&engine, "INSERT INTO left_t VALUES (2, NULL)");
    exec(&engine, "INSERT INTO right_t VALUES (10, 10)");
    exec(&engine, "INSERT INTO right_t VALUES (11, NULL)");

    // INNER JOIN on key_col: NULL never matches anything
    let result = engine.execute(
        "SELECT left_t.id FROM left_t JOIN right_t ON left_t.key_col = right_t.key_col"
    ).unwrap();
...
fn test_null_coalesce_in_select_col() {
    ...
    let result = engine.execute(
        "SELECT id, COALESCE(note, 'N/A') FROM t ORDER BY id"
    ).unwrap();
...
fn test_null_not_in_with_null_in_subquery() {
    ...
    let n = count_rows(&engine, "SELECT * FROM t WHERE val NOT IN (3, NULL)");
...
fn test_null_max_ignores_nulls() {
    ...
    exec(&engine, "CREATE TABLE t (val INT)");
    exec(&engine, "INSERT INTO t VALUES (NULL)");
    exec(&engine, "INSERT INTO t VALUES (7)");
    exec(&engine, "INSERT INTO t VALUES (NULL)");
    exec(&engine, "INSERT INTO t VALUES (2)");
...
fn test_null_update_where_is_null() {
    ...
    exec(&engine, "INSERT INTO t VALUES (1, 10)");
    exec(&engine, "INSERT INTO t VALUES (2, NULL)");
    exec(&engine, "INSERT INTO t VALUES (3, NULL)");

    engine.execute("UPDATE t SET val = 0 WHERE val IS NULL").unwrap();

    let n = count_rows(&engine, "SELECT * FROM t WHERE val = 0");
    assert_eq!(n, 2, "Both NULL rows should be updated to 0");
    let unchanged = query_int(&engine, "SELECT val FROM t WHERE id = 1");
