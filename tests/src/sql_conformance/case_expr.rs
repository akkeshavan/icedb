...
fn setup_case_tbl(engine: &std::sync::Arc<sql::engine::QueryEngine>) {
    exec(engine, "CREATE TABLE case_tbl (i INT, f FLOAT)");
    exec(engine, "INSERT INTO case_tbl VALUES (1, 10.1)");
    exec(engine, "INSERT INTO case_tbl VALUES (2, 20.2)");
    exec(engine, "INSERT INTO case_tbl VALUES (3, -30.3)");
    exec(engine, "INSERT INTO case_tbl VALUES (NULL, NULL)");

    exec(engine, "CREATE TABLE case2_tbl (i INT, j INT)");
    exec(engine, "INSERT INTO case2_tbl VALUES (1, NULL)");
    exec(engine, "INSERT INTO case2_tbl VALUES (2, 6)");
    exec(engine, "INSERT INTO case2_tbl VALUES (3, -6)");
    exec(engine, "INSERT INTO case2_tbl VALUES (2, -6)");
    exec(engine, "INSERT INTO case2_tbl VALUES (3, 6)");
}
...
fn test_case_simple_when() {
    ...
    let result = exec(&engine, "SELECT CASE 1 WHEN 1 THEN 'one' WHEN 2 THEN 'two' ELSE 'other' END");
...
fn test_case_simple_else() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = exec(&engine, "SELECT CASE 5 WHEN 1 THEN 'one' WHEN 2 THEN 'two' ELSE 'other' END");
    match result.rows[0].get_by_idx(0) {
...
fn test_case_searched_positive() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = exec(&engine, "SELECT CASE WHEN 5 > 3 THEN 'yes' ELSE 'no' END");
    match result.rows[0].get_by_idx(0) {
...
fn test_case_searched_false_branch() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = exec(&engine, "SELECT CASE WHEN 1 > 5 THEN 'yes' ELSE 'no' END");
    match result.rows[0].get_by_idx(0) {
...
fn test_case_null_result() {
    ...
    let result = exec(&engine, "SELECT CASE WHEN 1 = 2 THEN 'match' END");
    match result.rows[0].get_by_idx(0) {
...
fn test_case_string_categorization() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    exec(&engine, "CREATE TABLE scores (id INT, score INT)");
    exec(&engine, "INSERT INTO scores VALUES (1, 95), (2, 75), (3, 55), (4, 35)");
    let result = exec(&engine,
...
fn test_coalesce_first_non_null() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = exec(&engine, "SELECT COALESCE(NULL, NULL, 42, 99)");
    match result.rows[0].get_by_idx(0) {
        Some(Value::Int4(42)) => {}
...
fn test_coalesce_all_null() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = exec(&engine, "SELECT COALESCE(NULL, NULL, NULL)");
    match result.rows[0].get_by_idx(0) {
        Some(Value::Null) | None => {}
...
fn test_coalesce_single_value() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = exec(&engine, "SELECT COALESCE(7)");
    match result.rows[0].get_by_idx(0) {
        Some(Value::Int4(7)) => {}
...
fn test_nullif_equal() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = exec(&engine, "SELECT NULLIF(1, 1)");
    match result.rows[0].get_by_idx(0) {
        Some(Value::Null) | None => {}
...
fn test_nullif_not_equal() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = exec(&engine, "SELECT NULLIF(1, 2)");
    match result.rows[0].get_by_idx(0) {
        Some(Value::Int4(1)) => {}
...
fn test_nullif_text() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    let result = exec(&engine, "SELECT NULLIF('hello', 'hello')");
    match result.rows[0].get_by_idx(0) {
        Some(Value::Null) | None => {}
...
fn test_case_in_aggregate() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    exec(&engine, "CREATE TABLE votes (voter_id INT, candidate TEXT)");
    exec(&engine, "INSERT INTO votes VALUES (1, 'A'), (2, 'B'), (3, 'A'), (4, 'A'), (5, 'B')");
    let result = exec(&engine,
        "SELECT \
...
fn test_case_update_with_case() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    exec(&engine, "CREATE TABLE prices (id INT, price FLOAT, category TEXT)");
    exec(&engine, "INSERT INTO prices VALUES (1, 10.0, 'basic'), (2, 50.0, 'premium'), (3, 100.0, 'luxury')");
    exec(&engine,
        "UPDATE prices SET price = CASE \
           ...
           ELSE price \
         END");
    let result = exec(&engine, "SELECT id, price FROM prices ORDER BY id");
...
fn test_coalesce_from_table() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    exec(&engine, "CREATE TABLE maybe_null (id INT, val INT, default_val INT)");
    exec(&engine, "INSERT INTO maybe_null VALUES (1, NULL, 99), (2, 42, 99), (3, NULL, 77)");
    let result = exec(&engine, "SELECT id, COALESCE(val, default_val) AS effective_val FROM maybe_null ORDER BY id");
    assert_eq!(result.rows.len(), 3);
    match result.rows[0].get_by_idx(1) {
        Some(Value::Int4(99)) => {}
...
fn test_case_with_boolean_result() {
    let dir = TempDir::new().unwrap();
    let engine = make_engine(dir.path());
    exec(&engine, "CREATE TABLE tasks (tid INT, priority TEXT)");
    exec(&engine, "INSERT INTO tasks VALUES (1, 'high'), (2, 'low'), (3, 'medium'), (4, 'high')");
